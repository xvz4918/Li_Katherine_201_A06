...
[https://github.com/xvz4918/Li_Katherine_201_A06](https://github.com/xvz4918/Li_Katherine_201_A06)
...
```{r}
vec_list_ppg <- sapply(list_ppg, round, 2)
vec_list_ppg
...
mat_round <- function(mat_val) {
  if (nchar(mat_val) == 2) {
    return(round(mat_val, -1))
  } else if (nchar(mat_val) == 3) {
      return(round(mat_val, -2))
  } else {
    return(round(mat_val, -3))
  }
}

rounded_mat_rand <- apply(mat_rand, MARGIN = 1:2, mat_round)
rounded_mat_rand
```
...
```{r}
new_max_temps <- max_temps[, -c(1, 2)]
avg_max_temps <- apply(new_max_temps, 2, mean)
avg_max_temps
...
```{r}
data_co_max_temps <- co_max_temps[-c(1, 2)]
co_temp_data <- apply(data_co_max_temps, 1, median)
co_max <- max(co_temp_data)
co_min <- min(co_temp_data)
co_max_yr <- co_max_temps$year[[which.max(co_temp_data)]]
co_min_yr <- co_max_temps$year[[which.min(co_temp_data)]]
print(paste0("The maximum median annual temperature of ", co_max, " occurred in ", co_max_yr, " and the minimum median annual temperature of ", co_min, " occurred in ", co_min_yr, "." ))
```
...
```{r}
# sum_fx(935068) = 8 + sum_fx(93506) 
# sum_fx(93506) = 6 + sum_fx(9350)
# sum_fx(9350) = 0 + sum_fx(935)
# sum_fx(935) = 5 + sum_fx(93)
# sum_fx(93) = 3 + sum_fx(9)
# sum_fx(9) = 9 + sum_fx(0)

# base: num == 0
# recursive: last_digit + sum_fx(trunc_num)
...
sum_fx <- function(num) {
  if (num == 0) {
    ...
    return(last_digit + sum_fx(trunc_num))
  }
}
